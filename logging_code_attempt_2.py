...
for i in range(len(T1w)):
    ...
    h1r.append(cp.PropsSI ('H','T',T1r[i],'P',p1[i],"R134a"))
    h2r.append(cp.PropsSI ('H','T',T2r[i],'P',p2[i],"R134a"))
    h3r.append(cp.PropsSI ('H','T',T3r[i],'P',p2[i],"R134a")) # currently assuming no pressure drop through heat exchanger, will need to deal with this 
    h4 = h3r[i]
